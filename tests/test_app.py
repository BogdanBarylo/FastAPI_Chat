...
async def test_create_chat(client, redis, monkeypatch):
    ...
    assert chat_data == {"name": "test_chat", "chat_id": "CHT:test_chat_id", "ts": "2024-11-11T13:36:40"}
...
async def test_create_message(client, redis, monkeypatch, test_data):
    ...
    response = await client.post('/chats/CHT:test_id/messages', json={"text": "Hi"})
    assert response.status_code == 200
    message_data = await redis.hget(
        f'chat:CHT:test_id:message', "MSG:test_message_id"
    )
    message_data = json.loads(message_data)
    assert message_data == {"chat_id": "CHT:test_id", "text": "Hi", "message_id": "MSG:test_message_id", "ts": "2024-11-11T13:38:40"}
...
async def test_get_messages(client, test_data):
    ...
    assert response.status_code == 200
    response_data = response.json()
    assert response_data == {"messages":[{"chat_id": "CHT:test_id", "message_id": "MSG:test_id_1", "text": "hi, its test 1!", "ts": "2024-11-11T13:37:40"},
        {"chat_id": "CHT:test_id", "message_id": "MSG:test_id_2", "text": "hello again, test 2!", "ts": "2024-11-11T13:39:00"}]}
